...
Vector3 PosController::process(Vector3 ref_pos_NWU, State state, float dt){
    const float rad_yaw = TO_RAD*state.rpy(2);
    const float yaw_cos = cos(rad_yaw);
    const float yaw_sin = sin(rad_yaw);

    const Vector3 pos_diff_NWU = ref_pos_NWU - state.pos;  // global position diff
    ...
    const Vector3 local_ref = state.pos + pos_diff; 
